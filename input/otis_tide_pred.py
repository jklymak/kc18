...
def tide_pred(modfile, lon, lat, time, conlist=None):
    ...
    depth : water depth [m]
        depth according to OTIS at this location.  Can be useful for masking
        bad data.
	...
	u_re, u_im, v_re, v_im, h_re, h_im, omega, depth, conlist = extract_HC(
            modfile, lon, lat, conlist=conlist)
	...
	szo = (nt,)+sz
	return h.reshape(szo), u.reshape(szo), v.reshape(szo), depth
...
def extract_HC(modfile, lon, lat, z=None, conlist=None):
	...
	return u_re.reshape(szout), u_im.reshape(szout), v_re.reshape(szout), \
	    v_im.reshape(szout), h_re.reshape(szout), h_im.reshape(szout),  \
        omega, z, conlist
